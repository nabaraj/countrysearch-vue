...
describe("My First Test", () => {
  ...
  it("Visit country inner page", () => {
    let ahref = "";
    cy.get("#app > div > div > div > div:nth-child(2) > div:nth-child(1)").find("a")
      .then(($atag) => {
        ahref = $atag.attr('href')
      })
    cy.get("#app > div > div > div > div:nth-child(2) > div:nth-child(1)").find("a").click();
    cy.url().should('contain', ahref);
  })
  it("Check theme toggle", () => {
    cy.get("#themeChanger").click();
    cy.get("#themeChanger").get('span').then(($span) => {
      let text = $span.text();
      if (text === "light mode") {
